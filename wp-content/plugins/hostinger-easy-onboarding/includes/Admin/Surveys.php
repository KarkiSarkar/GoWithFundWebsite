...
class Surveys {
	...
	public function isAiOnboardingSurveyEnabled(): bool {
		...
		$isHostingerAdminPage       = $helper->isThisPage( 'hostinger' );
		$isWebsiteOlderThanTwoWeeks = ! $this->checkSiteCreationDateAndDisplayCsat();

		if ( ! $isAiOnboardingPassed || $isWebsiteOlderThanTwoWeeks ) {
			return false;
		}
	...
	public function checkSiteCreationDateAndDisplayCsat(): bool {
		global $wpdb;
		$oldest_user = $wpdb->get_var("SELECT user_registered FROM $wpdb->users ORDER BY user_registered ASC LIMIT 1");

		if ($oldest_user) {
			$oldest_user_timestamp = strtotime($oldest_user);
			$current_timestamp = current_time('timestamp');
			$weeks_difference = ($current_timestamp - $oldest_user_timestamp) / (7 * 24 * 60 * 60);
